...
void main() {
  ...
  group('getAllProducts', () {
    final jsonList = json.decode(readJson('helpers/dummy_data/product_cached_response.json'));
    
    final testProductModelList = ProductModel.fromJsonList(jsonList);

    
    
    const Cached_Poducts = 'CACHED_PRODUCTS';
    
    test(
      'should return cached products from shared reference when there is one in the cache',
      () async {
      //arrange
      when(mockSharedPreferences.getString(any)).thenReturn(json.encode(jsonList));

      //act
      final result = await dataSource.getAllProducts();

      //assert
      verify(mockSharedPreferences.getString(Cached_Poducts));
